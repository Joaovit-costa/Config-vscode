...
function displaySeparator(separator) {
    return separator === '\t' ? '\\t' : separator;
}
function parseSeparatorInput(raw) {
    return raw === '\\t' ? '\t' : raw;
}
...
function registerCsvCommands(context) {
    context.subscriptions.push(vscode.commands.registerCommand('csv.toggleExtension', () => toggleBooleanConfig('enabled', true, 'CSV extension')), vscode.commands.registerCommand('csv.toggleClickableLinks', () => toggleBooleanConfig('clickableLinks', true, 'CSV clickable links')), vscode.commands.registerCommand('csv.toggleHeader', async () => {
        const active = CsvEditorProvider_1.CsvEditorProvider.getActiveProvider();
        if (!active) {
            vscode.window.showInformationMessage('Open a CSV/TSV/PSV file in the CSV editor.');
            return;
        }
        ...
        const uri = active.getDocumentUri();
        const currentOverride = CsvEditorProvider_1.CsvEditorProvider.getSeparatorForUri(context, uri);
        const currentSep = currentOverride !== null && currentOverride !== void 0 ? currentOverride : active.getCurrentSeparator();
        ...
        if (input !== undefined) {
            ...
            await CsvEditorProvider_1.CsvEditorProvider.setSeparatorForUri(context, uri, sep);
            vscode.window.showInformationMessage(sep && sep.length
        ...
        const uri = active.getDocumentUri();
        await CsvEditorProvider_1.CsvEditorProvider.setSeparatorForUri(context, uri, undefined);
